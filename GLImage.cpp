...
Image loadImage(const char *fileName)
{
	...
	int size = 3 * bmih.biHeight * bmih.biWidth;
	unsigned char* data = new unsigned char[size];
	fread(data, sizeof(unsigned char), size, file);
	fclose(file);

	float* pixels = new float[bmih.biWidth*bmih.biHeight];
	int counter = 0;
	for (int i = bmih.biHeight - 1; i >= 0; i--) {
		for (int a = 0; a < bmih.biWidth; a++) {
			float val = data[(bmih.biHeight * i + a) * 3] + data[(bmih.biHeight * i + a) * 3 + 1] +
				data[(bmih.biHeight * i + a) * 3 + 2] + 0.0f;
			val = val / 3.f;
			if (val < 0) {
				val = 256.0 + val;
			}
	...
	Image i(pixels, bmih.biWidth, bmih.biHeight);
	return i;
}
